...
interface StudentLifeClientProps {
  moments: StudentLife[];
}
    ...
    >
      ...
      >
        {item.type === "video" && ytid ? (
          <div className="relative w-full aspect-video max-h-[80vh] rounded-2xl overflow-hidden shadow-2xl">
            <iframe
...
/* ── Client Component ──────────────────────────────────────────────────────────── */
export default function StudentLifeClient({ moments }: StudentLifeClientProps) {
  const [activeFilter, setActiveFilter] = useState("All");
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);
  const [layout, setLayout] = useState<"bento" | "grid">("bento");

  // Format static fallback items
  ...
  const spans: Array<"hero" | "tall" | "normal" | "wide"> = [
    "hero",
    "tall",
    "normal",
    "wide",
  ...
  // Map Sanity moments if available
  const hasSanityMoments = moments.length > 0;
  const mappedSanityItems: MediaItem[] = moments.map((moment, idx) => {
    const isYoutube = moment.mediaType === "youtube";
    const src = isYoutube ? (moment.youtubeUrl || "") : (moment.image ? urlFor(moment.image).url() : "");
    const ytid = isYoutube ? getYoutubeId(src) : null;
    
    let thumb = undefined;
    if (moment.customThumbnail) {
      thumb = urlFor(moment.customThumbnail).url();
    ...
    return {
      ...
      src,
      thumb,
      alt: moment.description || moment.title,
      category: moment.category,
      span: spans[idx % spans.length],
    };
  });

  const mediaItems = hasSanityMoments ? mappedSanityItems : fallbackItems;
